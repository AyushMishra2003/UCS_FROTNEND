    ...
    return (
        <div className='w-full px-[4vw] lg:px-[6vw] flex flex-col items-center justify-center bg-[#F5F6F7] py-20 pt-6'>
            ...
            <div className='w-fit flex flex-col sm:block p-4 bg-[#cedfe427] rounded-lg shadow-md'>
                ...
                <p className='text-base text-[#535760] mb-4' dangerouslySetInnerHTML={{ __html: children[0]?.description || "Description Not Available" }} />

                <h2 className='text-xl md:text-2xl font-semibold text-main mb-1'>
                    {children[1]?.title}
                </h2>
                <p className='text-base text-[#535760] mb-4' dangerouslySetInnerHTML={{ __html: children[1]?.description || "Description Not Available" }} />

                <h2 className='text-xl md:text-2xl font-semibold text-main mb-1'>
                    {children[2]?.title}
                </h2>
                <p className='text-base text-[#535760] mb-4' dangerouslySetInnerHTML={{ __html: children[2]?.description || "Description Not Available" }} />
                ...
                <p className='text-base text-[#535760] mb-4' dangerouslySetInnerHTML={{ __html: children[3]?.description || "Description Not Available" }} />

                <h2 className='text-xl md:text-2xl font-semibold text-main mb-1'>
                    {children[4]?.title}
                </h2>
                <p className='text-base text-[#535760] mb-4' dangerouslySetInnerHTML={{ __html: children[4]?.description || "Description Not Available" }} />
            </div>
        </div>
    );
};
